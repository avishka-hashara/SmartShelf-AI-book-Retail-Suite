// resources/js/Pages/PurchaseOrders/poPrint.js
//
// Builds a print-friendly A4 document for a Purchase Order and prints it via
// a hidden iframe (same approach as the POS receipt printer).
...
export const buildPurchaseOrderPrintHtml = (po, storeSettings = {}) => {
    const shopName = storeSettings.shop_name || 'Store';
    const logoUrl = storeSettings.logo_path ? `${window.location.origin}/storage/${storeSettings.logo_path}` : null;
    const storeLines = [
        storeSettings.address_line1,
        storeSettings.address_line2,
        [storeSettings.city, storeSettings.postal_code].filter(Boolean).join(' '),
        storeSettings.phone,
        storeSettings.email,
    ].filter(Boolean);
    ...
    const itemsMarkup = (po.items ?? []).map((item, idx) => `
        <tr>
            <td class="num">${idx + 1}</td>
            <td>
                <div class="item-name">${escapeHtml(item.product?.name ?? 'Item')}</div>
                <div class="item-sku">${escapeHtml(item.product?.sku ?? '')}</div>
            </td>
            <td class="num">${escapeHtml(String(item.qty_ordered))}</td>
            <td class="num">${escapeHtml(String(item.qty_received))}</td>
            <td class="num">${escapeHtml(fmt(item.unit_cost))}</td>
            <td class="num">${escapeHtml(fmt(item.qty_ordered * item.unit_cost))}</td>
        </tr>
    `).join('');
    ...
    <div class="doc">
        <div class="header">
            <div class="shop-block">
                ...
                <div>
                    ...
                    ${storeLines.length > 0 ? `<div class="shop-meta">${storeLines.map(l => escapeHtml(l)).join('<br />')}</div>` : ''}
                </div>
            </div>
            <div class="doc-title">
                <h1>Purchase Order</h1>
                <div class="po-number">${escapeHtml(po.po_number)}</div>
                <div class="status-badge">${escapeHtml(STATUS_LABELS[po.status] ?? po.status)}</div>
            </div>
        </div>
        ...
        <div class="meta-grid">
            <div class="meta-box">
                ...
                ${supplierLines.length > 0 ? `<p>${supplierLines.map(l => escapeHtml(l)).join('<br />')}</p>` : ''}
            </div>
            <div class="meta-box">
                <h3>Ordered By</h3>
                <p class="name">${escapeHtml(po.creator?.name ?? '—')}</p>
                ${supplier.payment_terms ? `<p>Payment Terms: ${escapeHtml(supplier.payment_terms)}</p>` : ''}
            </div>
        </div>
        ...
        <div class="dates-row">
            <div>
                ...
                <div class="value">${escapeHtml(formatDate(po.order_date))}</div>
            </div>
            <div>
                <div class="label">Expected Date</div>
                <div class="value">${escapeHtml(formatDate(po.expected_date))}</div>
            </div>
            <div>
                <div class="label">Received Date</div>
                <div class="value">${escapeHtml(formatDate(po.received_date))}</div>
            </div>
        </div>
        ...
        <div class="totals">
            <div class="totals-box">
                <div class="totals-row grand"><span>Total</span><span>${escapeHtml(fmt(po.total_cost))}</span></div>
            </div>
        </div>

        ${po.notes ? `<div class="notes"><h3>Notes</h3><p>${escapeHtml(po.notes)}</p></div>` : ''}
        ...
        <div class="footer">
            <span>Generated ${escapeHtml(new Date().toLocaleString('en-LK'))}</span>
...
export const printPurchaseOrder = (po, storeSettings = {}, { onAfterPrint } = {}) => {
    ...
    iframe.srcdoc = buildPurchaseOrderPrintHtml(po, storeSettings);
};
